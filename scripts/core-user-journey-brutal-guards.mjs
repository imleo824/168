...
const postPublishCache = read('src/features/post/postPublishCache.ts');
...
mustHave('post create selects the canonical client post shape', postCreateRoutes, 'select: PostService.listPostSelect(req.user.id)');
mustHave('post create returns a normalized client post', postCreateRoutes, 'PostService.toClientPost(post, req.user.id)');
...
mustHave('post create synchronizes publish caches after success', postCreatePage, 'synchronizePostPublishCache(queryClient, createdPost, user.id)');
mustHave('post publish cache clears persistent home feed snapshot', postPublishCache, 'clearHomeFeedSnapshots();');
mustHave('post publish cache seeds owner collections immediately', postPublishCache, 'seedPublishedPostForOwner(queryClient, post, viewerId);');
mustHave('post publish cache invalidates ranked collections in background', postPublishCache, "queryClient.invalidateQueries({ queryKey: ['posts'], refetchType: 'active' })");
mustHave('package core journey includes publish cache runtime guard', packageJson, 'npm run test:post-publish-cache');
