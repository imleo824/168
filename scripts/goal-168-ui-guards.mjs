...
assertIncludes('src/styles/tokens/profile-page-contracts.css', '--ui-profile-cover-pattern-opacity: 0.18;', 'profile cover texture overlay must stay subtle for photo clarity.');
...
assertIncludes('src/styles/system/ui-primitives-layout.css', '.app-shell {\n    box-sizing: border-box;', 'the app shell must include frame padding inside the viewport width and height.');
assertIncludes('src/styles/system/ui-primitives-layout.css', '.app-shell-main {\n    box-sizing: border-box;', 'the main panel must include borders and route padding inside its assigned grid size.');
assertIncludes('src/styles/system/wide-screen-mobile-adaptation.css', 'padding-block-end: var(--ui-space-none);', 'desktop main panels must not reserve hidden mobile bottom-navigation space.');
...
assertIncludes('src/styles/tokens/tui-plus-page-contracts.css', '--ui-plus-link-label-max-width:', 'plus and promotion link labels must use one semantic route-owned label width token.');
...
assertMatches('src/styles/system/wide-screen-mobile-adaptation.css', /@media \(min-width: 1024px\) and \(max-width: 1179px\)[\s\S]*?--app-desktop-aux-rail-width: var\(--ui-space-none\);[\s\S]*?--app-desktop-main-width: var\(--app-desktop-workspace-main-width\);[\s\S]*?--app-desktop-main-side-gap-total: var\(--app-desktop-shell-gap\);/, '1024-1179 desktop shell must keep a stable two-column frame without reserving advertising rail width.');
assertIncludes('src/app/AppShell.tsx', 'useIsDesktopViewport(UI_USER_DESKTOP_AUX_RAIL_MIN_WIDTH)', 'advertising rail mounting must follow the shared wide desktop breakpoint.');
assertIncludes('src/styles/features/aux-rail.css', '@media (max-width: 1179px)', 'advertising rail CSS must degrade below the wide desktop breakpoint.');
assertIncludes('src/styles/features/aux-rail.css', '@media (min-width: 1180px)', 'advertising rail CSS must activate only when enough desktop width exists.');
...
assertMatches('src/styles/system/wide-screen-mobile-adaptation.css', /\.app-desktop-sidebar\s*\{[\s\S]*?align-self:\s*stretch;/, 'desktop sidebar must fill the viewport-bound app frame row.');
assertNotIncludes('src/styles/system/wide-screen-mobile-adaptation.css', 'position: sticky;', 'desktop frame children must not add a second sticky offset inside the viewport-bound grid.');
...
assertIncludes('src/styles/tokens/profile-page-contracts.css', '--ui-profile-cover-image-opacity: 1;', 'profile cover images must stay fully opaque for clarity.');
...
assertIncludes('src/styles/tokens/promote-contracts.css', '--ui-promote-payment-sheet-max-height: calc(var(--ui-visual-viewport-height) - env(safe-area-inset-top) - var(--ui-space-3));', 'promote/payment sheet height must derive from the shared visual viewport contract.');
assertNotIncludes('src/styles/tokens/promote-contracts.css', '--ui-promote-payment-sheet-max-height: 88dvh', 'promote/payment sheet height must not use raw dynamic viewport units.');
assertIncludes('src/styles/tokens/promote-contracts.css', '--ui-promote-picker-sheet-max-height: calc(var(--ui-visual-viewport-height) - var(--ui-space-4));', 'promote picker sheet height must derive from the shared visual viewport contract.');
assertNotIncludes('src/styles/tokens/promote-contracts.css', '--ui-promote-picker-sheet-max-height: 86dvh', 'promote picker sheet height must not use raw dynamic viewport units.');
...
assertIncludes('src/styles/system/ui-primitives-auth-modal.css', '--ui-auth-agreement-panel-max-height: clamp(calc(var(--ui-space-8) * 4), calc(var(--ui-visual-viewport-height) - (var(--ui-space-8) * 19)), calc(var(--ui-space-8) * 7));', 'auth agreement panel height must derive from the visual viewport contract.');
assertNotIncludes('src/styles/system/ui-primitives-auth-modal.css', '34svh', 'auth agreement panel height must not use raw small viewport sizing.');
assertIncludes('src/styles/system/ui-primitives-auth-modal.css', 'max-width: calc(var(--ui-viewport-width) - var(--ui-space-6));', 'compact auth panel width must use the shared viewport width token.');
assertNotIncludes('src/styles/system/ui-primitives-auth-modal.css', 'calc(100vw - var(--ui-space-6))', 'compact auth panel width must not use raw viewport width.');
