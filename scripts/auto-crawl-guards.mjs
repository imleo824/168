...
const crawlMedia = read('server/services/auto-crawl-media.service.ts');
...
const backfillMigration = read('prisma/migrations/20260917000000_add_auto_crawl_backfill_state/migration.sql');
...
mustHave('domain types', types, /backfillBeforeCursor/);
mustHave('domain types', types, /backfillTargetCursor/);
...
mustHave('shared publish contract', publishContract, /export type PostPublishDraft/);
mustHave('shared publish contract', publishContract, /export type PreparedPostPublishData/);
mustHave('shared publish contract', publishContract, /normalizePublishCategoryMetaPayload/);
mustHave('shared publish contract', publishContract, /category: \{ connect: \{ id: prepared\.category\.id \} \}/);
mustHave('shared publish contract', publishContract, /export async function createPreparedPost/);
mustHave('main flow', crawl, /preparePostPublishData/);
mustHave('main flow', crawl, /categoryMeta: extracted\.meta/);
mustHave('main flow', crawl, /createPreparedPost\(tx, prepared/);
mustNotHave('main flow', crawl, /tx\.post\.create/);
mustHave('main flow', crawl, /set_config\('app\.auto_crawl_write','1',true\)/);
mustHave('media persistence', crawl, /persistAutoCrawlImages/);
mustHave('media persistence', crawlMedia, /assertPublicHttpTarget/);
mustHave('media persistence', crawlMedia, /IMAGE_MAX_BYTES/);
mustHave('media persistence', crawlMedia, /IMAGE_CONCURRENCY = 2/);
mustHave('media persistence', crawlMedia, /canonicalizeImageUrl/);
mustHave('media persistence', crawlMedia, /contentHash = sha256\(fetched\.buffer\)/);
mustHave('media retry reuse', crawlMedia, /mapping: Record<string, string>/);
mustHave('media retry reuse', crawl, /persistedImageMap: error\.audit\.mapping/);
mustHave('atomic item publish', crawl, /"status"='PUBLISHED'[\s\S]{0,240}"postId"=\$4/);
mustHave('stable publish nonce', crawl, /auto-crawl:\$\{hash/);
mustHave('bounded retry claim', crawlRecovery, /"retryCount"="retryCount"\+1/);
mustHave('bounded retry delay', crawlRecovery, /"retryCount"=0 THEN INTERVAL '5 minutes'/);
mustHave('bounded retry delay', crawlRecovery, /"retryCount"=1 THEN INTERVAL '30 minutes'/);
mustHave('bounded retry delay', crawlRecovery, /ELSE INTERVAL '2 hours'/);
mustHave('backfill schema', prismaSchema, /backfillBeforeCursor String\?/);
mustHave('backfill schema', prismaSchema, /backfillTargetCursor String\?/);
mustHave('backfill migration', backfillMigration, /ADD COLUMN IF NOT EXISTS "backfillBeforeCursor"/);
mustHave('backfill migration', backfillMigration, /ADD COLUMN IF NOT EXISTS "backfillTargetCursor"/);
mustHave('backfill persistence', crawl, /"backfillBeforeCursor"=\$15,"backfillTargetCursor"=\$16/);
mustHave('raw persistence cursor safety', crawl, /if \(rawStoreFailed\) \{\s*stats\.cursor = initialCursor/);
mustHave('clean runtime cutover migration', backfillMigration, /DELETE FROM public\."AutoCrawlItem"/);
