...
export default function ReferralInvitePageContent({ isRulesOpen, onCloseRules }: ReferralInvitePageContentProps) {
  ...
  useScrollLock(isRulesOpen || isWithdrawSheetOpen || isConvertSheetOpen, { fixed: true, allowTouchMove: (target) => target instanceof Element && Boolean(target.closest('[data-chat-rules-scroll], [data-payment-action-scroll], [data-promote-sheet-scroll]')) });
