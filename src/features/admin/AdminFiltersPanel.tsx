...
import { RefreshCcw, SlidersHorizontal, ChevronDown, ChevronUp } from 'lucide-react';
import {
...
export function AdminFiltersPanel({
  ...
  const hasSearchKeyword = activeTab !== 'deposit-addresses' || activeDepositAddressSection === 'list';
    ...
    return (
      <div className="flex items-center gap-2 w-full lg:hidden mb-2">
        ...
        <button
          ...
          className="pressable flex items-center justify-center gap-1.5 min-h-[var(--ui-admin-filter-control-height)] px-3.5 border border-[var(--ui-line-medium)] rounded-[var(--ui-radius-xl)] bg-white text-[var(--ui-text-secondary)] font-bold text-xs"
        >
          <SlidersHorizontal size={14} />
          <span>{isMobileExpanded ? '收起筛选' : '高级筛选'}</span>
          {isMobileExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
        </button>
      </div>
    );
  };
  ...
  return (
    <div className="admin-filter-bar">
      ...
      <div className={`${isMobileExpanded || !hasSearchKeyword ? 'block' : 'hidden'} lg:block w-full`}>
        {activeTab === 'content' && (
          <div className="admin-filter-grid admin-filter-grid--four">
            <input
              type="text"
              className="admin-filter-control hidden lg:block"
              value={searchKeyword}
              onChange={(e) => setSearchKeyword(e.target.value)}
              placeholder="搜索标题或正文"
            />
        ...
        {activeTab === 'transactions' && (
          <div className="admin-filter-grid admin-filter-grid--five">
            <input
              type="text"
              className="admin-filter-control admin-filter-control--span-2 hidden lg:block"
              value={searchKeyword}
              onChange={(e) => setSearchKeyword(e.target.value)}
        ...
        <div className="admin-filter-query-row mt-2.5">
