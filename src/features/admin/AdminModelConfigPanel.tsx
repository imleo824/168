...
export function AdminModelConfigPanel() {
  ...
  const fetchModelConfig = useCallback(async () => {
    setIsLoading(true);
    try {
      const [commentRes, quoteRes] = await Promise.all([
        apiFetch('/api/admin/comment-publish/config'),
        apiFetch('/api/admin/quote-publish/config'),
      ]);
      if (!commentRes.ok || !quoteRes.ok) throw new Error('模型配置加载失败');
      const nextConfigs = {
        comment: await commentRes.json(),
        quote: await quoteRes.json(),
      };
      setConfigs(nextConfigs);
      setDraftModel(String(nextConfigs.comment?.model || nextConfigs.quote?.aiModel || DEFAULT_MODEL));
    } catch (error: any) {
  ...
  const saveModelConfig = async () => {
    ...
    const baseConfigs = configs || { comment: {}, quote: {} };
    setIsSaving(true);
    try {
      const [commentRes, quoteRes] = await Promise.all([
        apiFetch('/api/admin/comment-publish/config', {
      ...
      if (!commentRes.ok || !quoteRes.ok) throw new Error('模型配置保存失败');
      const nextConfigs = {
        comment: await commentRes.json(),
        quote: await quoteRes.json(),
      };
      setConfigs(nextConfigs);
      setDraftModel(String(nextConfigs.comment?.model || nextConfigs.quote?.aiModel || model));
      ...
      <div className="admin-system-config-header">
        <div className="admin-system-config-title-group">
          ...
          <div className="admin-system-config-copy">
            ...
            <p className="admin-system-config-summary">统一维护自动评论、自动引用使用的模型。</p>
          </div>
        </div>
        <button
          type="button"
